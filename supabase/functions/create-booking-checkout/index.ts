...
serve(async (req) => {
  ...
  try {
    ...
    if (!eventId || !attendeeName || !attendeeEmail) {
      throw new Error("Missing required fields: eventId, attendeeName, attendeeEmail");
    }
    ...
    if (!event.price || event.price <= 0) throw new Error("Event price not set");
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${req.headers.get("origin")}/events?booking=canceled`,
      metadata: {
        eventId,
        userId: user.id,
        attendeeName,
